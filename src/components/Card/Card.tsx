...
const Card = ({ product, variant }: { product: Product; variant?: string }) => {
	...
	return (
		<div className='relative'>
			...
			<button
				className={`absolute flex items-center justify-center top-0 right-0 w-12 h-12  ${
					variant === "summary" ? "lg:w-8 lg:h-8" : "lg:w-16 lg:h-16"
				}`}
				onClick={() => toggleFavorites(product)}>
				{isFavorite(product.id) ? (
					<IoHeartSharp size={variant === "summary" ? 20 : 24} color='#ef4da2' />
				) : (
